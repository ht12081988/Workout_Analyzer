...
export default function SessionDetailPage() {
  ...
  const [activeTab, setActiveTab] = useState<'performance' | 'attempts'>('performance');
                ...
                <div className="space-y-4 max-h-[600px] overflow-y-auto pr-4 custom-scrollbar">
                  {session.attempts.length === 0 ? (
                    ...
                    session.attempts.map((attempt) => (
                      <div
                        key={attempt.id}
                        className="flex items-center justify-between rounded-[2rem] bg-surface-container-lowest p-6 transition-all hover:translate-x-2"
                      >
                        <div className="flex items-center gap-4">
